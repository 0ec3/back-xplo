 ...
 * @since      1.0.0
 */
class revenue_pro_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			...
			'description' => __( 'Display popular posts by comments with thumbnails.', 'revenue-pro' )
		);

		// Create the widget.
		parent::__construct(
			'revenue_pro-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts', 'revenue-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'revenue_pro_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'revenue_pro_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'revenue_pro_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'revenue-pro' ),
			'limit'     => 5,
			'show_views' => true
		);
