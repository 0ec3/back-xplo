 ...
 * Set import files
 */
function course_set_import_files() {
    return array(
        array(
            'import_file_name'           => __('Demo content', 'revenueplus'),
...
add_filter( 'pt-ocdi/import_files', 'course_set_import_files' );

/**
 * Define actions that happen after import
 */
function course_set_after_import_mods() {

	//Assign the menu
    $primary_menu = get_term_by( 'name', 'Primary', 'nav_menu' );
    //$secondary_menu = get_term_by( 'name', 'Secondary Menu', 'nav_menu' );
    $footer_menu = get_term_by( 'name', 'Primary', 'nav_menu' );
...
add_action( 'pt-ocdi/after_import', 'course_set_after_import_mods' );
