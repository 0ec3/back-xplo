 ...
 * @since      1.0.0
 */
class revenueplus_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-revenueplus-popular widget_posts_thumbnail',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'revenueplus' )
		);

		// Create the widget.
		parent::__construct(
			'revenueplus-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts', 'revenueplus' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'revenueplus_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'revenueplus_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'revenueplus_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'revenueplus' ),
			'limit'     => 5,
			'show_date' => true
		);
