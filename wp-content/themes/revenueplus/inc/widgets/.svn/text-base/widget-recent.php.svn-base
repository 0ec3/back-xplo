 ...
 * @since      1.0.0
 */
class revenue_pro_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-revenue_pro-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'revenue-pro' )
		);

		// Create the widget.
		parent::__construct(
			'revenue_pro-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'revenue-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'revenue_pro_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'revenue_pro_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'revenue_pro_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'revenue-pro' ),
			'limit'     => 5,
			'show_date' => true
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>">
				<?php _e( 'Number of posts to show', 'revenue-pro' ); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_date' ); ?>">
				<?php _e( 'Display post date?', 'revenue-pro' ); ?>
			</label>
		</p>
