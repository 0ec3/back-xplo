 ...
 * @since      1.0.0
 */
class revenueplus_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-revenueplus-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'revenueplus' )
		);

		// Create the widget.
		parent::__construct(
			'revenueplus-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'revenueplus' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'revenueplus_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'revenueplus_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'revenueplus_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'revenueplus' ),
			'limit'     => 5,
			'show_date' => true
		);
